...
It does so by, basically, successively routing from one sampling point to the next with its :ref:`underlying router <candidate_router>`.

But if, at any point, it is not able to find a :term:`valid navigation route`,
it is able to go back to previous sampling points (up to the configured maximum) in history and re-route from other candidates.
That means we try out other routes that were not considered optimal in the first place.
